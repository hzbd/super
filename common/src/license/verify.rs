...
/// Default kid used by some issuers when claims omit `kid` (Manager products use `k_*`).
...
// `PUBLIC_KEY_BYTES` + `PUBLIC_KEY_RING` — every file under `common/keys/`
// (`{product}.{kid}.public.key`). Generated by build.rs; populate with `make fetch-keys`.
